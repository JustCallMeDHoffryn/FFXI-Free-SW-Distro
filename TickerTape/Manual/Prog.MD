...
If **B** is set to -1 then the size of the target object is used, so in this example it would be 1 (byte), but if it was accessing a word or rword the increment would be 2 etc.
